...
test.serial('Lista de usuário', async t => {
	await create()
...
test.serial('Criação de usuário', async t => {
	const result = await create()
  t.is(result.user.email, 'user@test.com')
})

test.serial('Atualização de usuário', async t => {
	await create()
  const updated = await users.update(1, '123456789')
  t.is(updated.affectedRows, 1)
})

test.serial('Remoção de usuário', async t => {
  await create()
